...
{
    class HTTPStatusCodeLookupExample
    {
        private static readonly string API_KEY = Environment.GetEnvironmentVariable("APIVERVE_API_KEY") ?? "YOUR_API_KEY_HERE";
        private static readonly string API_URL = "https://api.apiverve.com/v1/httpstatuslookup";

        /// <summary>
        /// Make a GET request to the HTTP Status Code Lookup API
        /// </summary>
        static async Task<JsonDocument> CallHTTPStatusCodeLookupAPI()
        {
            try
            {
                ...
                // Query parameters
                var queryParams &#x3D; new Dictionary&lt;string, string&gt; { [&quot;code&quot;] &#x3D; 404 };
        ...
        {
            Console.WriteLine("📤 Calling HTTP Status Code Lookup API...\n");

            var result = await CallHTTPStatusCodeLookupAPI();
